...
export function TabletDayView({ 
  ...
  const getCurrentTimePosition = () => {
    ...
    // Calcular la posición en píxeles (cada slot de 30 min = 60px para tablet)
    const totalMinutes = hours * 60 + minutes
    const slotIndex = totalMinutes / 30 // Cada slot es de 30 minutos
    return slotIndex * 60 // 60px por slot para tablet
  }
  ...
  const handlePointerMove = (e: React.PointerEvent) => {
    if (dragStartPos && !isPointerDragging) {
      const deltaX = Math.abs(e.clientX - dragStartPos.x)
      const deltaY = Math.abs(e.clientY - dragStartPos.y)
      
      // Si se ha movido más de 5px, considerar como drag
  ...
  const handleEventClick = (event: CalendarEvent) => {
    ...
    if (!isPointerDragging && timeDiff < 200) {
      onEventClick?.(event)
    }
  }
        ...
        <div className="flex flex-col flex-1">
          ...
          <div className="flex-1 overflow-auto">
            <div 
              ...
              style={{ 
                height: '2880px', // 48 slots * 60px = 2880px
                touchAction: 'none'
              }}
            >
              ...
              {isToday && (
                ...
                >
                  {/* Círculo indicador */}
                  <div className="w-3 h-3 bg-primary rounded-full border-2 border-background shadow-md ml-20 z-50"></div>
                  {/* Línea horizontal */}
              ...
              <div className="absolute left-20 top-0 w-px bg-border/30 z-15" style={{ height: '2880px' }}></div>
              ...
              {timeSlots.filter(slot => !slot.isHalfHour && slot.hour !== 0).map((slot) => {
                ...
                return (
                  <div
                    key={`label-${slot.hour}`}
                    className="absolute left-0 w-20 text-right pr-4 z-20 flex items-center justify-end"
                    style={{ 
